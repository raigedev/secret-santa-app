...
- Use Stitch as the main design source for this Secret Santa app. For any screen, flow, or component we are designing or materially redesigning, ask Stitch to create or refresh the design direction first, then collaborate with Figma, Oracle, Taste/Impeccable skills, browser preview, and Playwright as supporting tools. Always use Stitch and Figma MCP design tools when the user asks for UI design context, design tokens, screen metadata, selected frames, screenshots, variants, or implementation details from connected design sources. For this app's current Secret Santa Stitch source, use project `3072957204541081703` (`Process Explainer`, despite the generic title) after confirming with `list_projects`/`list_screens` when needed; do not call `list_design_systems` without a `projectId`. For Figma, use the connected Figma MCP/plugin automatically when available: start with exact-node design context, fetch a screenshot/reference for the selected frame, reuse this repo's existing components/tokens, and verify the implementation with browser or Playwright checks. When the task needs design exploration, mockups, or design-system sync, use Figma write/design tools if an editable file or frame is available; if access is view-only, use Figma as a supporting reference source and continue with Stitch, `DESIGN.md`, local code, and Playwright/browser verification. If Figma tools are not exposed in the current Codex session, say that briefly and continue from screenshots, Stitch, `DESIGN.md`, and local code.
...
GSD (`get-shit-done-cc`) is available as a local Codex workflow aid. Use its phase-style workflow automatically when a task benefits from structured discussion, planning, execution, or verification, especially broad or ambiguous implementation work. Keep it subordinate to these repo instructions: do not let it bypass VS Code Problems checks, tests/build, Oracle/Figma/Stitch rules, Supabase safety rules, manual commit/push preference, or the Codex requirement that sub-agents are only spawned when the user explicitly asks for sub-agents or parallel agent work. Keep generated GSD files such as `.codex/get-shit-done/`, `.codex/skills/gsd-*`, and `.planning/` out of commits unless the user explicitly asks to vendor them. For GSD updates, use explicit Codex local minimal flags such as `npx.cmd --yes get-shit-done-cc@latest --codex --local --minimal`; do not run plain non-interactive installs that can default to another runtime.
...
Use `sqlmap` only after the tool source is reviewed or verified. Do not execute the ignored `.agent/tools/sqlmap/sqlmap.py` path automatically; prefer a reviewed pinned copy or official release, and only run it for authorized local dev or app-owned preview SQL injection checks when endpoint, query, form, or route-param testing would benefit from it. Keep it defensive and scoped: low-risk/read-only options by default, no data dumping, no tamper/evasion, no broad crawling, and never scan production, third-party, or user-supplied external systems without explicit approval. Keep auth cookies, tokens, and captured payloads out of chat, docs, logs, screenshots, and commits; treat findings as leads to verify in code, tests, and Supabase/RLS checks.
...
- Use Google Stitch as the primary UI design source for this app's screen direction, design tokens, screen metadata, and implementation details. For meaningful new or redesigned UI, ask Stitch to create or refresh the design direction before implementing; then use local code, Figma references, Oracle/Taste review, and Playwright/browser screenshots to refine and verify. Start from `list_projects`, then call project-scoped tools such as `list_design_systems` or `list_screens`; the known Secret Santa Stitch source is project `3072957204541081703`, currently titled `Process Explainer`.
...
- Use CodeRabbit or an equivalent AI code reviewer on every pull request only when it is available and confirmed non-billable for this workspace. Do not enable paid plans, usage-based add-ons, credits, auto-refill, subscriptions, or Slack Agent billing from Codex. If the current CodeRabbit plan or usage-based add-on state cannot be verified as free/non-billable, skip CodeRabbit and rely on the required local checks, GitHub checks, CodeQL, Dependency Review, Vercel, and human diff review instead. Treat CodeRabbit as an additional security/reliability signal for SQL injection, exposed credentials, broken auth, unsafe redirects, race conditions, and other common regressions; it is not a replacement for tests, typecheck, linting, build, or human review.
- When using `sqlmap`, do not blindly trust an ignored local checkout and do not execute `.agent/tools/sqlmap/sqlmap.py` automatically. Verify the tool path and provenance first, prefer a reviewed pinned copy or official release, and run it through the bundled Codex Python runtime only for scoped local/app-owned targets. Use it alongside Playwright, PayloadsAllTheThings references, security skills, and manual code review for targeted input-validation checks; do not let scanner output replace authorization, rate-limit, or RLS verification.
...
- When the user says `done push` for this repo, run the usual verified PR flow instead of stopping at push confirmation: fetch the remote branch, verify the pushed commits and working tree, inspect the relevant diff, confirm the required checks or run the matching local validation, create or reuse the `dev` -> `main` pull request, wait for required GitHub checks, merge only when the PR is safe and checks pass, sync `dev` back to the merge commit, leave the workspace on `dev`, and report the PR URL, merge commit, validation, and close notes.
- Never blindly merge a pushed branch. If checks are pending or failing, the diff does not match the requested fix, Supabase/database migration state is unclear, unrelated changes are present, or the risk cannot be validated from the available evidence, stop and explain the blocker instead of merging.
- At the start of coding/debugging work, and again before final handoff, run `npm.cmd run check:problems` or the most relevant Problems scanner available. Treat any VS Code Problems popup as actionable until the scanner proves the repo is clean. If VS Code shows a diagnostic that `check:problems` misses, fix the source diagnostic and update `scripts/check-vscode-problems.mjs` or the matching lint/cSpell/config rule so future runs catch the same pattern automatically.
