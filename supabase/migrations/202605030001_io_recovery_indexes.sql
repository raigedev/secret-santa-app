...
create index if not exists groups_event_date_id_idx
  on public.groups (event_date, id);

create index if not exists assignments_group_pending_giver_created_idx
...
create index if not exists messages_group_thread_created_idx
...
create index if not exists messages_group_sender_idx
  on public.messages (group_id, sender_id);

create index if not exists thread_reads_user_thread_idx
