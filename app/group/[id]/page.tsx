...
type WishlistItem = {
  id: string; group_id: string; item_name: string;
  item_link: string; item_note: string; priority: number;
};

type RecipientData = {
  group_id: string; group_name: string; group_event_date: string;
  receiver_nickname: string; receiver_wishlist: WishlistItem[];
  gift_received: boolean; gift_received_at: string | null;
};
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const [
        ...
        supabase.from("assignments").select("group_id, receiver_id").eq("giver_id", user.id).in("group_id", groupIds),
        supabase.from("assignments").select("group_id, gift_received, gift_received_at").eq("receiver_id", user.id).in("group_id", groupIds),
      ]);
      ...
      if (myAssignments && myAssignments.length > 0) {
        ...
        const [
          { data: receiverMembers },
          { data: receiverWishlists },
        ] = await Promise.all([
          supabase.from("group_members").select("group_id, user_id, nickname").in("user_id", receiverIds).in("group_id", groupIds).eq("status", "accepted"),
          supabase.from("wishlists").select("id, group_id, user_id, item_name, item_link, item_note, priority").in("user_id", receiverIds).in("group_id", groupIds),
        ]);

        const recipientData: RecipientData[] = myAssignments.map((a) => {
          const group = (groupsData || []).find((g) => g.id === a.group_id);
          const receiver = (receiverMembers || []).find((m) => m.user_id === a.receiver_id && m.group_id === a.group_id);
          const wishlist = (receiverWishlists || [])
            .filter((w) => w.user_id === a.receiver_id && w.group_id === a.group_id)
            .map((w) => ({ id: w.id, group_id: w.group_id, item_name: w.item_name, item_link: w.item_link || "", item_note: w.item_note || "", priority: w.priority || 0 }));
      ...
      const { data: myWishlistData } = await supabase
        .from("wishlists").select("id, group_id, item_name, item_link, item_note, priority")
        .eq("user_id", user.id).in("group_id", groupIds);

      setMyItems(
        (myWishlistData || []).map((w) => ({ id: w.id, group_id: w.group_id, item_name: w.item_name, item_link: w.item_link || "", item_note: w.item_note || "", priority: w.priority || 0 }))
      );
    ...
    const channel = supabase.channel("santa-page-realtime")
    ...
    <main className="min-h-screen relative overflow-x-hidden" style={{ background: "linear-gradient(180deg,#1a0a0a 0%,#2d1010 20%,#3d1515 50%,#2d1010 80%,#1a0a0a 100%)", fontFamily: "'Nunito', sans-serif", color: "#fff" }}>
      ...
      <div className="relative z-10 max-w-[720px] mx-auto px-4 py-6">
        ...
        <div className="text-center mb-7">
          <h1 className="text-[32px] font-bold mb-1" style={{ fontFamily: "'Fredoka', sans-serif", textShadow: "0 2px 8px rgba(0,0,0,.3)" }}>🎅 Your Secret Santa</h1>
          <p className="text-[14px] font-semibold" style={{ color: "rgba(255,255,255,.5)" }}>Your mystery recipients from all events</p>
              ...
              <div key={i} className="rounded-[18px] overflow-hidden transition"
                ...
                <div className="flex items-center justify-between p-4" style={{ background: "rgba(255,255,255,.03)", borderBottom: "1px solid rgba(255,255,255,.06)" }}>
                  <div className="flex items-center gap-3">
                    <div className="w-[44px] h-[44px] rounded-xl flex items-center justify-center text-[22px]"
                      style={{ background: iconColors[i % iconColors.length] }}>{iconEmojis[i % iconEmojis.length]}</div>
                    <div>
                ...
                <div className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-[14px] font-extrabold" style={{ color: "rgba(255,255,255,.7)" }}>🎅 {a.receiver_nickname}&apos;s wishlist</p>
                  ...
                  {a.receiver_wishlist.length === 0 ? (
                    <div className="text-center py-5 rounded-xl" style={{ color: "rgba(255,255,255,.2)", border: "1px dashed rgba(255,255,255,.08)" }}>
                      No wishlist items yet 😢<br /><span className="text-[11px]" style={{ opacity: .6 }}>{a.receiver_nickname} hasn&apos;t added any gift ideas</span>
                    </div>
                  ) : (
                  ...
                  <div className="rounded-xl p-3.5 mt-3 flex items-center justify-between"
                    style={{
                      background: a.gift_received ? "rgba(34,197,94,.06)" : "rgba(251,191,36,.06)",
                      border: `1px solid ${a.gift_received ? "rgba(34,197,94,.12)" : "rgba(251,191,36,.12)"}`,
                    }}>
                    <div>
                      <div className="text-[13px] font-extrabold" style={{ color: a.gift_received ? "#22c55e" : "#fbbf24" }}>
                        {a.gift_received ? "✅ Gift Received!" : "🎁 Did you receive your gift?"}
                      </div>
                      <div className="text-[11px] mt-0.5" style={{ color: "rgba(255,255,255,.35)" }}>
                        {a.gift_received
                          ? `Confirmed on ${new Date(a.gift_received_at || "").toLocaleDateString()}`
                          : "Your Secret Santa bought you something! Confirm when you get it."
                        }
                      </div>
                    </div>
                    {a.gift_received ? (
                      <div className="px-4 py-2 rounded-xl text-[12px] font-bold" style={{ background: "rgba(34,197,94,.1)", color: "#22c55e" }}>✅ Confirmed</div>
                    ) : (
                      <button onClick={() => handleConfirmGift(a.group_id)}
                        disabled={confirmingGroup === a.group_id}
                        className="px-4 py-2 rounded-xl text-[12px] font-extrabold text-white transition"
                        style={{ background: "linear-gradient(135deg,#22c55e,#16a34a)", boxShadow: "0 3px 12px rgba(34,197,94,.3)", border: "none", cursor: "pointer", fontFamily: "inherit" }}>
                        {confirmingGroup === a.group_id ? "Confirming..." : "✅ I got my gift!"}
                      </button>
                    )}
                  </div>
                  ...
                  <button onClick={() => router.push(`/group/${a.group_id}`)}
                    ...
                    style={{ color: "#fbbf24", background: "rgba(251,191,36,.06)", border: "1px solid rgba(251,191,36,.1)", fontFamily: "inherit", cursor: "pointer" }}>
                    View group →
                  </button>
                </div>
              </div>
        ...
        <div className="rounded-[18px] overflow-hidden" style={{ background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.08)", backdropFilter: "blur(8px)" }}>
          <div className="flex items-center justify-between p-4" style={{ background: "rgba(255,255,255,.03)", borderBottom: "1px solid rgba(255,255,255,.06)" }}>
            <div className="flex items-center gap-3">
              <div className="w-[38px] h-[38px] rounded-lg flex items-center justify-center text-[18px]" style={{ background: "rgba(220,38,38,.15)" }}>📝</div>
              <div>
            ...
            <button onClick={() => setShowAdd(!showAdd)}
              ...
              style={{ background: "linear-gradient(135deg,#dc2626,#ef4444)", boxShadow: "0 2px 12px rgba(220,38,38,.25)", border: "none", cursor: "pointer", fontFamily: "inherit" }}>
              + Add Item
            </button>
          </div>
          ...
          <div className="p-4">
            {message && (
              <p className={`text-[11px] font-bold mb-3 ${message.includes("!") && !message.includes("required") ? "text-green-400" : "text-red-400"}`}>{message}</p>
            )}
            ...
            {showAdd && (
              <div className="rounded-xl p-4 mb-4" style={{ background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.08)" }}>
                ...
                <input value={addName} onChange={(e) => setAddName(e.target.value)} placeholder="Item name (e.g. Nintendo Switch)..."
                ...
                <div className="flex gap-2 mb-2">
                  <input value={addLink} onChange={(e) => setAddLink(e.target.value)} placeholder="Link (optional)..."
                ...
                <div className="flex items-center justify-between">
                  <label className="flex items-center gap-2 text-[11px] font-bold" style={{ color: "rgba(255,255,255,.4)" }}>
                    <input type="checkbox" checked={addPriority > 0} onChange={(e) => setAddPriority(e.target.checked ? 1 : 0)} style={{ accentColor: "#fbbf24" }} />
                    ⭐ Top priority
                  </label>
                  <div className="flex gap-2">
                    <button onClick={() => setShowAdd(false)} className="px-4 py-2 rounded-lg text-[11px] font-bold"
                    ...
                    <button onClick={handleAdd} disabled={addLoading} className="px-4 py-2 rounded-lg text-[11px] font-bold text-white"
                      style={{ background: "#22c55e", border: "none", fontFamily: "inherit", cursor: "pointer" }}>{addLoading ? "Saving..." : "✅ Save"}</button>
                  </div>
                </div>
              </div>
            )}
                    ...
                    <div className="flex items-center justify-between p-3 rounded-xl mb-2 transition"
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-[9px] font-bold mt-1 inline-block px-2 py-0.5 rounded"
                          style={{ color: "rgba(255,255,255,.25)", background: "rgba(255,255,255,.04)" }}>{getGroupName(item.group_id)}</div>
                      </div>
