...
// Tracks read status for accurate unread badges.
//
// Security: Core#1, Core#3, Playbook#09, #19
...
export default function SecretSantaChatPage() {
  ...
  useEffect(() => {
    const loadThreads = async () => {
      ...
      const groupIds = [
        ...new Set((memberRows || []).map((r) => r.group_id)),
      ];
      ...
      const receiverUserIds = (giverAssignments || [])
        .map((a) => a.receiver_id)
        .filter(Boolean);
      const giverUserIds = (receiverAssignments || [])
        .map((a) => a.giver_id)
        .filter(Boolean);
      ...
      const { data: allMessages } = await supabase
        .from("messages")
        .select(
          "group_id, thread_giver_id, thread_receiver_id, sender_id, content, created_at"
        )
        .in("group_id", groupIds)
        .order("created_at", { ascending: false });

      // Fetch read timestamps for unread calculation
      const { data: readTimestamps } = await supabase
        .from("thread_reads")
        .select(
          "group_id, thread_giver_id, thread_receiver_id, last_read_at"
        )
        .eq("user_id", user.id);
      ...
      for (const a of giverAssignments || []) {
        ...
        const receiver = memberNicknames.find(
          (m) =>
            m.user_id === a.receiver_id && m.group_id === a.group_id
        );
        const threadMsgs = (allMessages || []).filter(
          (m) =>
            m.group_id === a.group_id &&
            m.thread_giver_id === a.giver_id &&
            m.thread_receiver_id === a.receiver_id
        );
        const lastMsg = threadMsgs[0];
        const receiverName = receiver?.nickname || "Participant";

        // Calculate unread: messages from other person after last read
        const readEntry = (readTimestamps || []).find(
          (r) =>
            r.group_id === a.group_id &&
            r.thread_giver_id === a.giver_id &&
            r.thread_receiver_id === a.receiver_id
        );
        const lastRead = readEntry
          ? new Date(readEntry.last_read_at)
          : new Date(0);
        ...
        buildThreads.push({
          ...
          last_message: lastMsg
            ? (lastMsg.sender_id === user.id
                ? "You: "
                : `${receiverName}: `) + lastMsg.content.slice(0, 40)
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(
        `chat-${activeThread.group_id}-${activeThread.giver_id}-${activeThread.receiver_id}`
      )
      .on(
        "postgres_changes",
        {
  ...
  if (loading)
    return (
      <main
        ...
        style={{
          background: "linear-gradient(180deg,#0a1628,#162d50,#0a1628)",
        }}
      >
  ...
  if (activeThread) {
    ...
    return (
      <main
        ...
        style={{
          background:
            "linear-gradient(180deg,#0a1628 0%,#0f1f3d 20%,#162d50 50%,#0f1f3d 80%,#0a1628 100%)",
        ...
        <div className="relative z-10 max-w-[720px] mx-auto px-4 py-6">
          <div
            ...
            style={{
              ...
              border: `1px solid ${isGiver ? "rgba(251,191,36,.15)" : "rgba(34,197,94,.15)"}`,
            }}
          >
            {/* Chat header */}
            <div
              ...
              style={{
                background: "rgba(255,255,255,.04)",
                borderBottom: `1px solid ${isGiver ? "rgba(251,191,36,.1)" : "rgba(34,197,94,.1)"}`,
              }}
            >
              <div className="flex items-center gap-3">
                <div
                  className="w-[42px] h-[42px] rounded-xl flex items-center justify-center text-[18px]"
                  style={{
                    background: isGiver
                ...
                <div>
                  <div
                    ...
                    style={{ color: isGiver ? "#fbbf24" : "#86efac" }}
                  >
                  ...
                  >
                    ...
                    {isGiver &&
                      ` · ${activeThread.other_name} sees you as "🎅 Secret Santa"`}
                  </div>
                </div>
              </div>
              <button
              ...
              <span
                ...
                style={{
                  background: isGiver
                    ? "rgba(251,191,36,.12)"
                    : "rgba(34,197,94,.12)",
                  color: isGiver ? "#fbbf24" : "#86efac",
                }}
              >
                {isGiver
                  ? `🎁 You are ${activeThread.other_name}'s Secret Santa`
                  : "🎅 This person drew your name — identity hidden!"}
              </span>
            </div>
              ...
              {messages.length === 0 ? (
                ...
                messages.map((msg) => {
                  ...
                  return (
                    <div
                      key={msg.id}
                      className={`max-w-[75%] px-3.5 py-2.5 rounded-[14px] text-[13px] leading-relaxed ${isMine ? "self-end" : "self-start"}`}
                      style={{
                        background: isMine
                          ? isGiver
            ...
            <div
              ...
              style={{
                ...
                borderTop: `1px solid ${isGiver ? "rgba(251,191,36,.08)" : "rgba(34,197,94,.08)"}`,
              }}
            >
              <input
                value={msgInput}
                ...
                placeholder={
                  isGiver
                    ? `Message ${activeThread.other_name} as 🎅 Secret Santa...`
                ...
                style={{
                  ...
                  border: `1px solid ${isGiver ? "rgba(251,191,36,.12)" : "rgba(34,197,94,.12)"}`,
                  color: "#fff",
                  fontFamily: "inherit",
                }}
              />
              <button
                ...
                style={{
                  ...
                  fontFamily: "inherit",
                  cursor:
                    msgInput.trim() && !sending ? "pointer" : "not-allowed",
                  boxShadow:
                    msgInput.trim() && !sending
                      ? isGiver
                        ? "0 2px 10px rgba(180,83,9,.3)"
          ...
          {isGiver && (
            <p
              ...
              style={{ color: "rgba(255,255,255,.2)" }}
            >
              🔒 {activeThread.other_name} sees your messages as &quot;🎅
              Your Secret Santa&quot; — your identity stays hidden
            </p>
          )}
        </div>
      </main>
      ...
      <div className="relative z-10 max-w-[720px] mx-auto px-4 py-6">
        ...
        {threads.length === 0 ? (
          ...
          >
            ...
            >
              Once names are drawn, your chat threads will appear here!
            </p>
          </div>
        ) : (
          <>
            ...
            {giverThreads.length > 0 && (
              <>
                <div className="flex items-center gap-3 mb-3">
                  <div
                    className="w-[40px] h-[40px] rounded-xl flex items-center justify-center text-[20px]"
                    style={{
                  ...
                  <div>
                    ...
                    >
                      You know who they are — they don&apos;t know it&apos;s
                      you!
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-2.5 mb-6">
                  {giverThreads.map((t, i) => (
                    <div
                      ...
                      style={{
                        background:
                          "linear-gradient(135deg,rgba(251,191,36,.06),rgba(245,158,11,.04))",
                    ...
                    >
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        <div
                          className="w-[48px] h-[48px] rounded-[14px] flex items-center justify-center text-[20px] flex-shrink-0"
                          style={{
                            background:
                              "linear-gradient(135deg,#fbbf24,#f59e0b)",
                            boxShadow:
                              "0 3px 12px rgba(251,191,36,.25)",
                          }}
                        >
                          🎁
                        </div>
                ...
                <div className="flex items-center gap-3 mb-3">
                  <div
                    ...
                    style={{
                      ...
                      boxShadow: "0 2px 10px rgba(34,197,94,.1)",
                    }}
                  >
                    🎅
                  </div>
                  <div>
                    ...
                    <div
                      className="text-[11px] font-semibold"
                      style={{ color: "rgba(34,197,94,.5)" }}
                    >
                      Someone drew your name — you don&apos;t know who!
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-2.5">
                  {receiverThreads.map((t, i) => (
                    <div
                      ...
                      style={{
                        background:
                          "linear-gradient(135deg,rgba(34,197,94,.06),rgba(22,163,74,.04))",
        ...
        {/* Privacy note */}
        <div
          className="flex items-start gap-2 mt-6 p-3.5 rounded-xl"
          style={{
        ...
        >
          ...
          <div
            ...
            style={{ color: "rgba(147,197,253,.6)" }}
          >
            <strong style={{ color: "#93c5fd" }}>
              Your identity is always hidden
            </strong>{" "}
            when chatting with recipients. They only see &quot;🎅 Secret
            Santa&quot;. Your Secret Santa&apos;s identity is hidden from you
            too!
          </div>
        </div>
      </div>
  ...
  useEffect(() => {
    ...
    if (sw && sw.children.length === 0) {
      for (let i = 0; i < 50; i++) {
        ...
        s.style.cssText = `width:${sz}px;height:${sz}px;left:${Math.random() * 100}%;animation-duration:${5 + Math.random() * 10}s;animation-delay:${Math.random() * 6}s;opacity:${0.15 + Math.random() * 0.25};`;
        sw.appendChild(s);
      }
    }
    return () => {
