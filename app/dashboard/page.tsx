...
// ═══════════════════════════════════════
// DASHBOARD — Split Groups View
// Security: #02 Supabase Auth, #08 parameterized queries,
// #09 RLS enabled, #19 server-side checks via RLS
// Real-time: subscribes to group_members, groups, assignments
// ═══════════════════════════════════════
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      ...
      if (acceptedGroupIds.length > 0) {
        ...
        const drawnGroupIds = new Set((assignments || []).map((a) => a.group_id));
  ...
  const GroupCard = ({ group, type }: { group: Group; type: "owned" | "invited" }) => (
    <div
      ...
      style={{
        background: type === "owned"
          ? "linear-gradient(135deg,#1e40af,#2563eb)"
          : "linear-gradient(135deg,#b45309,#f59e0b)",
        boxShadow: type === "owned"
          ? "0 4px 20px rgba(37,99,235,.25)"
          : "0 4px 20px rgba(245,158,11,.25)",
      }}
    >
      <div className="flex items-center justify-between px-4 py-2.5" style={{ background: "rgba(255,255,255,.9)" }}>
        <span className="text-sm font-extrabold" style={{ color: type === "owned" ? "#1e40af" : "#b45309" }}>
          🎁 {group.name}
        </span>
        <span className="text-[9px] font-extrabold px-2 py-0.5 rounded-full uppercase tracking-wide"
          style={{
            background: type === "owned" ? "rgba(37,99,235,.1)" : "rgba(245,158,11,.1)",
      ...
      <div className="px-4 py-3 text-white">
        {group.description && <p className="text-xs opacity-85 mb-1.5">{group.description}</p>}
          ...
          <div className="mb-2.5">
            ...
            <div className="flex flex-wrap gap-1">
              {group.members.slice(0, 4).map((m, i) => (
                <span key={i} className="text-[10px] font-bold px-2.5 py-0.5 rounded-full" style={{ background: "rgba(255,255,255,.2)" }}>
                  {m.nickname || "Anonymous"}
                </span>
              ))}
              {group.members.length > 4 && (
                <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full" style={{ background: "rgba(255,255,255,.15)" }}>
      ...
      <div className="relative z-10 text-center max-w-5xl w-full p-10 rounded-xl shadow-xl bg-white/40 backdrop-blur-md">
          ...
          {ownedGroups.length === 0 ? (
            <div className="text-center py-5 rounded-xl" style={{ background: "rgba(0,0,0,.02)", border: "1px dashed rgba(0,0,0,.08)" }}>
              ...
              <button onClick={() => router.push("/create-group")}
                ...
                style={{ background: "linear-gradient(135deg,#2563eb,#3b82f6)", boxShadow: "0 2px 10px rgba(37,99,235,.25)" }}>
                + Create Your First Group
              </button>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {ownedGroups.map((group) => <GroupCard key={group.id} group={group} type="owned" />)}
            </div>
          )}
        </div>
        ...
        <div className="text-left mb-10">
          ...
          {invitedGroups.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {invitedGroups.map((group) => <GroupCard key={group.id} group={group} type="invited" />)}
            </div>
          )}
        </div>
