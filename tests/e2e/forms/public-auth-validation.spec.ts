...
test.describe("public auth form validation", () => {
  ...
  test("create-account validates email format", async ({ page }) => {
    ...
    await page.getByPlaceholder(/your email/i).fill("not-an-email");
    await page.getByPlaceholder(/your password/i).fill("12345678");
    await page.getByRole("button", { name: /sign up/i }).click();
  ...
  test("create-account enforces a minimum password length", async ({ page }) => {
    ...
    await page.getByPlaceholder(/your name/i).fill("Playwright Tester");
    await page.getByPlaceholder(/your email/i).fill("tester@example.com");
    await page.getByPlaceholder(/your password/i).fill("short");
    await page.getByRole("button", { name: /sign up/i }).click();
  ...
  test("forgot-password requires an email before submission", async ({ page }) => {
    ...
    await expect(page.getByText(/please enter your registered email/i)).toBeVisible();
  });

  test("reset-password enforces the minimum password length", async ({ page }) => {
    await page.goto("/reset-password");

    await page.getByPlaceholder(/enter new password/i).fill("short");
